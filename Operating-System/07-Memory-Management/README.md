...
First Fit: Uses 500K hole → [100K, 288K, 200K, 300K, 600K]
Best Fit:  Uses 300K hole → [100K, 500K, 200K, 88K, 600K]
Worst Fit: Uses 600K hole → [100K, 500K, 200K, 300K, 388K]
```
...
**Calculate:**
- Page offset bits: 12 (since 4KB = 2¹²)
- Page number bits: 16 - 12 = 4 → 16 pages
- Frame offset bits: 12
- Frame number bits: log₂(32KB/4KB) = log₂(8) = 3
...
3. If page 3 maps to frame 5:
   Physical address = 5 × 4096 + 1212 = 21692
...
```
┌───────────────────────────────────┐
│ Page Number (20) │ Offset (12)    │  32-bit address, 4KB pages
└───────────────────────────────────┘
         │
         ↓
┌─────────────────────────────────┐
│       Page Table (2²⁰ entries)  │  4MB for table!
└─────────────────────────────────┘
```
...
│   Hash Table                       │
│  ┌─────┬─────────┬───────┬─────┐  │
│  │Index│ VPN     │ Frame │ Next│  │
│  ├─────┼─────────┼───────┼─────┤  │
│  │  h  │   p     │   f   │  →  │  │ Chain for collisions
│  └─────┴─────────┴───────┴─────┘  │
└───────────────────────────────────┘
```
...
**Calculate:**
- Number of pages = 2³²/2¹² = 2²⁰ = 1M pages
- Page table size = 1M × 4B = 4MB
...
- **I**nternal → **I**nside (fixed blocks, waste inside)
- **E**xternal → **E**verywhere (scattered holes)
...
**Right:** Offset bits = log₂(page size), same in both addresses
...
| Page Table Size | (VA Space / Page Size) × Entry Size |
